...
def favicon_url(url, size: 64, override: nil)
  return override if override.to_s.strip != ""

  query = URI.encode_www_form(sz: size, domain_url: url)
  "https://www.google.com/s2/favicons?#{query}"
end
...
def link_icon(item)
  ...
  when "freshrss" then icon(:rss)
  when "linkding" then icon(:linkding)
  when "youtube" then icon(:youtube)
  else
    icon(:link)
  end
end
...
def site_rss_link
  %(<a href="/atom.xml" class="rss-link" aria-label="RSS feed">#{icon(:rss)} RSS</a>)
end
